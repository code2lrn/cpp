...
static_assert(static_cast<char>(-1) < 0, "Signed char required");

//static_assert(sizeof(int) == sizeof(int*), "Expect same size for pointers and ints");

#include <type_traits>
#include <algorithm>
template <class T>
unsigned char* raw_copy(const T &val, unsigned char *buf) {
   static_assert(std::is_trivially_copyable<T>, "Function requires bitwise copyable type");
